...
if [ -f "/app/alembic/env.py" ]; then
  # 修复env.py中的meta_data拼写错误，如果存在
  sed -i 's/meta_data/metadata/g' /app/alembic/env.py
  echo "已修复env.py中的拼写错误"
fi
...
if [[ $PG_COUNT == *"0"* ]]; then
  ...
  if [[ $VERSION_COUNT == *"0"* ]]; then
    ...
    # 获取第一个迁移文件
    FIRST_MIGRATION=$(ls -1 /app/alembic/versions/ | grep -i "initial" | head -n 1 | sed 's/.py//')
